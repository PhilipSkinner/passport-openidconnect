...
function Strategy(options, verify) {
  ...
  if (!this._options.iatSeconds) {
    this._options.iatSeconds = 5;
  }
}
  ...
  if (req.query && req.query.code) {
    ...
    function loaded(err, ok, state) {
      ...
      oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: callbackURL }, function(err, accessToken, refreshToken, params) {
        ...
        //five seconds ago and in the future
        var startDate = (Date.now() / 1000) - parseInt(self._options.iatSeconds) * 1000;
        var endDate = (Date.now() / 1000) + parseInt(self._options.iatSeconds) * 1000;

        if (jwtClaims.iat > endDate || jwtClaims.iat < startDate) return self.error(new Error('id token not received quickly enough'));
