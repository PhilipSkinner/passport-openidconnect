...
describe('strategy', function() {
  ...
  describe('configured to work with a specific OpenID provider', function() {
    ...
    describe('issuing authorization request', function() {
      ...
      describe('that redirects to identity provider with prompt option', function() {
        var strategy = new Strategy({
          issuer: 'https://www.example.com',
          authorizationURL: 'https://www.example.com/oauth2/authorize',
          tokenURL: 'https://www.example.com/oauth2/token',
        ...
        before(function(done) {
          chai.passport.use(strategy)
            ...
            .authenticate({ prompt: 'none' });
        });
  
        it('should be redirected', function() {
          expect(url).to.equal('https://www.example.com/oauth2/authorize?response_type=code&client_id=ABC123&redirect_uri=https%3A%2F%2Fwww.example.net%2Flogin%2Freturn&scope=openid&prompt=none&state=' + encodeURIComponent(state));
        });
        ...
        it('should save state in session', function() {
          ...
          expect(request.session['openidconnect:www.example.com'].state.clientSecret).to.equal('secret');
          expect(request.session['openidconnect:www.example.com'].state.params.response_type).to.equal('code');
          expect(request.session['openidconnect:www.example.com'].state.params.prompt).to.equal('none');
        });
      }); // that redirects to identity provider with prompt as an option

      describe('that redirects to identity provider with prompt configuration', function() {
        var strategy = new Strategy({
          issuer: 'https://www.example.com',
          authorizationURL: 'https://www.example.com/oauth2/authorize',
          tokenURL: 'https://www.example.com/oauth2/token',
